...
Discord moderators apply \textbf{graduated punishments} — warnings, deletions, timeouts, bans —
based on the full trajectory of a user's behaviour. Automated systems collapse this to a binary
\textit{remove}/\textit{permit} decision and ignore user history.\\[2mm]

Across \textbf{13 languages}, multilingual toxicity classifiers (XLM-R) carry systematic
per-language bias: Arabic averages $-0.139$ below English-equivalent content; Italian,
Ukrainian, and German are under-scored; Hinglish is over-scored by $+0.095$. Threshold-based
moderation bakes this bias into every decision.
\end{minipage}\hfill
...
\node[draw, rounded corners, fill=muaccent!85, text=white, minimum width=78mm, minimum height=22mm, right=of t] (b) {BAN};
...
% Both blocks use identical total height via matched content + padding
% ============================================================
\begin{columns}
...
Static threshold & 0.770 & 0.000 & 0.000 & {\color{muaccent}\textbf{$\times$}} \\
\rowcolor{mubg}\textbf{RL agent} & \textbf{0.736} & 0.153 & $\mathbf{10^{-4}}$ & {\color{mugreen}\textbf{\checkmark}} \\
\bottomrule
\end{tabular}
\end{center}
...
% ============================================================
% ROW 5 : By the Numbers  (full width, colourful stat boxes)
% Block 7 (Trajectory) removed — redundant with per-class bars in Block 5,
% and was pushing "By the Numbers" off the bottom of the A0 page.
% ============================================================
{\colorlet{blockbodybgcolor}{munavy}%
\colorlet{blockbodyfgcolor}{white}%
\begin{columns}
\column{1}
\block{}{% ============================================================
% ROW 5 : Key stats as coloured banners on a navy block background
% ============================================================
\newcommand{\stathex}[3]{%
    % #1 = colour, #2 = big number, #3 = small label
